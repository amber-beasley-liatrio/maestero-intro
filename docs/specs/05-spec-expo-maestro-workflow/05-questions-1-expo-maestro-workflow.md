...
- [ ] (A) **Separate workflows**: Create `maestro-test-ios-expo-reusable.yml` for Expo, keep existing workflow for React Native CLI
...
- [ ] (A) **pnpm only**: Hardcode pnpm usage, require Expo projects to use pnpm
...
- [ ] (A) **Workflow secrets**: Accept environment variables as workflow secrets (e.g., `bundle-id` secret input)
...
- [ ] (A) **Add working-directory input**: Configurable input (default: "." for root, allows "apps/native" override)
...
- [ ] (A) **Increase default to 40 minutes**: Change `timeout-minutes` input default from 30 to 40
...
- [ ] (A) **Cache after prebuild**: Cache `{working-directory}/ios/Pods` with hash of `{working-directory}/ios/Podfile.lock`
...
- [ ] (B) **Change to "tests/maestro/"**: Match common Expo monorepo pattern (relative to working-directory)
...
- [ ] (B) **Full workflow simulation**: Execute all workflow steps locally (prebuild, build, Metro, all tests, artifacts)
